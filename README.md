...
- **Machines** — name, **hostname / FQDN / IP**, what it does (role), status, and optional
  OS/location/notes
- **Ports** — per machine: number + tcp/udp + what it serves + **what it connects to** — add
  one or more of your machines (each picked from a dropdown, or "external"), with an optional
  port/detail per connection, so a single port can be open for several machines. Every target
