...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/":
            ...
            self._send(200, json.dumps({"path": self.server.data_path}))
...
def main():
    ...
    ap.add_argument("--port", type=int, default=8099, help="loopback port (default 8099)")
...
<header class="sticky top-0 z-10 flex flex-wrap items-center gap-3 px-4 py-2.5 bg-bg/85 backdrop-blur border-b border-line">
  ...
  <nav class="flex gap-1">
    ...
    <button class="tab" data-tab="ports">port&nbsp;map</button>
  </nav>
...
async function load(){
  try{ S=await (await fetch("api/state")).json(); }catch(e){ S={machines:[]}; }
  ...
  let _mig=false;
  S.machines.forEach(m=>{ if(Array.isArray(m.tickets)){ m.tickets.forEach(tk=>{ tk.machine=m.name; S.tickets.push(tk); _mig=true; }); delete m.tickets; } });
  if(_mig) save();
  ...
  if(["machines","tickets","ports"].includes(h)) TAB=h;
...
function matchM(m){ if(!Q)return true; const q=Q.toLowerCase();
  return [m.name,m.role,m.host,m.os,m.location,m.notes,...(m.ports||[]).flatMap(p=>[p.number,p.proto,p.serves,p.connectsTo,p.connects])].join(" ").toLowerCase().includes(q);
}
...
function setF(i,k,v){ if(k==="name"){const old=S.machines[i].name;S.machines[i].name=v;(S.tickets||[]).forEach(t=>{if(t.machine===old)t.machine=v;});S.machines.forEach(mm=>(mm.ports||[]).forEach(p=>{if(p.connectsTo===old)p.connectsTo=v;}));} else {S.machines[i][k]=v;} save();statline(); }
...
function delM(i){const nm=S.machines[i].name;if(confirm("Delete "+(nm||"this machine")+"?")){(S.tickets||[]).forEach(t=>{if(t.machine===nm)t.machine="";});S.machines.forEach(mm=>(mm.ports||[]).forEach(p=>{if(p.connectsTo===nm)p.connectsTo="";}));S.machines.splice(i,1);save();render();}}
function addPort(i){S.machines[i].ports.push({number:0,proto:"tcp",serves:"",connectsTo:"",connects:""});save();render();}
...
function connectOpts(p){ return `<option value="" ${!p.connectsTo?"selected":""}>— none</option>`+S.machines.map(m=>`<option value="${esc(m.name)}" ${p.connectsTo===m.name?"selected":""}>${esc(m.name)}</option>`).join("")+`<option value="external" ${p.connectsTo==="external"?"selected":""}>external</option>`; }
function incomingFor(name){ const r=[]; S.machines.forEach(m=>(m.ports||[]).forEach(p=>{ if(p.connectsTo===name) r.push({p,from:m}); })); return r; }
...
function render(){ statline(); (TAB==="tickets"?renderTickets:TAB==="ports"?renderPorts:renderMachines)(); }
      ...
      <div class="flex flex-col gap-2">${(m.ports||[]).map((p,j)=>`
        <div class="port flex flex-col gap-1">
          ...
          <div class="flex items-center gap-1.5 text-[0.82rem] pl-[1.2rem]">
            ...
            <select class="field font-sans py-1 text-[0.82rem]" onchange="setPort(${i},${j},'connectsTo',this.value)" title="connects to which machine">${connectOpts(p)}</select>
            <input class="flat text-dim flex-1" value="${esc(p.connects)}" oninput="setPort(${i},${j},'connects',this.value)" placeholder="port / detail (optional)">
          </div>
        </div>`).join("")}
        <button class="addrow" onclick="addPort(${i})">+ port</button>
      </div>
      ${inc.length?`<div class="eyebrow">incoming</div>
      <div class="flex flex-col gap-1">${inc.map(({p,from})=>`
        <div class="flex items-center gap-2 text-[0.85rem]" title="opened by a connection from ${esc(from.name)} — edit it on that machine">
          ...
          <span class="text-dim truncate min-w-0">← ${esc(from.name)}${from.host?` <span class="text-faint">(${esc(from.host)})</span>`:""}${p.connects?` <span class="text-faint">${esc(p.connects)}</span>`:""}</span>
        </div>`).join("")}
      </div>`:""}
...
function renderPorts(){
  const rows=[]; S.machines.forEach(m=>(m.ports||[]).forEach(p=>rows.push({n:p.number||0,proto:p.proto||"tcp",serves:p.serves||"",mn:m.name||"",connectsTo:p.connectsTo||"",connects:p.connects||""})));
  ...
  const vis=rows.filter(r=>!Q||(r.n+" "+r.proto+" "+r.serves+" "+r.mn+" "+r.connectsTo+" "+r.connects).toLowerCase().includes(Q.toLowerCase()));
  ...
  app.innerHTML=`<table class="w-full border-collapse text-[0.9rem]"><thead><tr><th class="${th}">port</th><th class="${th}">proto</th><th class="${th}">machine</th><th class="${th}">serves</th><th class="${th}">connects to</th></tr></thead><tbody>`+
    vis.map(r=>{const re=count[key(r)]>1;return `<tr>
      ...
      <td class="${td} font-mono text-dim">${(r.connectsTo||r.connects)?`${r.connectsTo?`<span class="text-ink">${esc(r.connectsTo)}</span>`:""}${r.connects?` ${esc(r.connects)}`:""}`:`<span class="text-faint">—</span>`}</td></tr>`;}).join("")+`</tbody></table>`;
}
