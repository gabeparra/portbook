...
function matchM(m){ if(!Q)return true; const q=Q.toLowerCase();
  return [m.name,m.role,m.host,m.os,m.location,m.notes,...(m.ports||[]).flatMap(p=>[p.number,p.proto,p.serves,p.connects])].join(" ").toLowerCase().includes(q);
}
...
function setF(i,k,v){ if(k==="name"){const old=S.machines[i].name;S.machines[i].name=v;(S.tickets||[]).forEach(t=>{if(t.machine===old)t.machine=v;});} else {S.machines[i][k]=v;} save();statline(); }
...
function delM(i){const nm=S.machines[i].name;if(confirm("Delete "+(nm||"this machine")+"?")){(S.tickets||[]).forEach(t=>{if(t.machine===nm)t.machine="";});S.machines.splice(i,1);save();render();}}
function addPort(i){S.machines[i].ports.push({number:0,proto:"tcp",serves:"",connects:""});save();render();}
    ...
    <div class="card group">
      ...
      <div class="flex flex-col gap-2">${(m.ports||[]).map((p,j)=>`
        <div class="port flex flex-col gap-1">
          ...
          <input class="flat text-dim text-[0.82rem] pl-[5.4rem]" value="${esc(p.connects)}" oninput="setPort(${i},${j},'connects',this.value)" placeholder="↳ connects to (host / machine:port)">
        </div>`).join("")}
...
function renderPorts(){
  const rows=[]; S.machines.forEach(m=>(m.ports||[]).forEach(p=>rows.push({n:p.number||0,proto:p.proto||"tcp",serves:p.serves||"",mn:m.name||"",connects:p.connects||""})));
  ...
  const vis=rows.filter(r=>!Q||(r.n+" "+r.proto+" "+r.serves+" "+r.mn+" "+r.connects).toLowerCase().includes(Q.toLowerCase()));
  ...
  app.innerHTML=`<table class="w-full border-collapse text-[0.9rem]"><thead><tr><th class="${th}">port</th><th class="${th}">proto</th><th class="${th}">machine</th><th class="${th}">serves</th><th class="${th}">connects to</th></tr></thead><tbody>`+
    vis.map(r=>{const re=count[key(r)]>1;return `<tr>
      ...
      <td class="${td} font-mono text-dim">${r.connects?esc(r.connects):`<span class="text-faint">—</span>`}</td></tr>`;}).join("")+`</tbody></table>`;
}
